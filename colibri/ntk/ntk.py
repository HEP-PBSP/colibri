...
def ntk_ensemble_at_epoch(
  ...
  with ThreadPoolExecutor(max_workers=max_workers) as executor:
      future_to_replica = {
          executor.submit(
              compute_ntk_at_epoch_for_replica,
              replica_idx
          ): replica_idx
      ...
      for future in tqdm(
          futures_iter, total=n_replicas, desc="Computing NTKs"
      ):
...
def ntk_at_epoch(ntk_ensemble_at_epoch, flavours):
    ...
    if len(flavours) > shape[0]:
            raise ValueError(
                f"flavour_indices {flavours} out of range [0, {shape[0]})"
            )
    
    dfs = [pd.DataFrame(
        data=ntk_ensemble[k], index=index, columns=index) 
