...
def plot_perm_test(mc_fit, bayesian_fit, perm_settings):
    ...
    if not "statistic" in perm_settings:
        raise KeyError("The 'statistic' key must be present in the perm_settings dictionary.")

    if perm_settings["statistic"] == "mean":
        perm_settings["statistic"] = lambda x, y, axis: np.mean(x, axis=axis) - np.mean(y, axis=axis)
        name_statistic = "mean"
    elif perm_settings["statistic"] == "var":
        perm_settings["statistic"] = lambda x, y, axis: np.var(x, axis=axis) - np.var(y, axis=axis)
    ...
    for i, param in enumerate(parameter_names):
        ...
        ax.axvline(res.statistic[i], color="red", linestyle="--", label=f"Statistic {param}")
        ax.set_title(f'Statistic: {name_statistic} \n Permutation test p-value: {p_value[i]:.2f}')
        ax.legend()
        yield fig
...
if __name__ == "__main__":
    ...
    perm_settings = {"n_resamples": 1000, "alternative": "two-sided", "statistic": "mean"}
