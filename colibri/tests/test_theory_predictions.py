...
from colibri.tests.conftest import TEST_DATASET, CLOSURE_TEST_PDFSET, TEST_DATASET_HAD, TEST_DATASETS, TEST_DATASETS_HAD
...
def test_make_dis_prediction():
    ...
    pdf_grid = colibriAPI.closure_test_pdf_grid(**{**CLOSURE_TEST_PDFSET, **TEST_DATASETS})

    fktable = load_fktable(ds.fkspecs[0])
    FIT_XGRID = colibriAPI.FIT_XGRID(**TEST_DATASETS)
    pred1 = make_dis_prediction(fktable, FIT_XGRID, vectorized=False, flavour_indices=None)(
        pdf_grid[0]
    )
...
def test_make_had_prediction():
    ...
    pdf_grid = colibriAPI.closure_test_pdf_grid(**{**CLOSURE_TEST_PDFSET, **TEST_DATASETS_HAD})

    fktable = load_fktable(ds.fkspecs[0])

    FIT_XGRID = colibriAPI.FIT_XGRID(**TEST_DATASETS_HAD)
    pred1 = make_had_prediction(fktable, FIT_XGRID, vectorized=False, flavour_indices=None)(
        pdf_grid[0]
    )
