...
** TO DO** Add some introductory theory on Hessian fits.
...
.. code-block:: bash
    ...
    max_epochs: 30000            # The fraction of the data used for validation in training.
    patience: 30000
...
``hessian_settings``
^^^^^^^^^^^^^^^^^^^^
* ``ErrorType``: You can chose between ``hessian`` and ``replicas``. **TO DO: explain what each does**
* ``tolerance``: **TODO**
* ``n_samples``: Number of parameter sets to generate for uncertainty propagation. This is only used if ``ErrorType`` is set to ``replicas``
...
* ``rng_seed``: Controls the random number generator seed for reproducibility. This is only used if ``ErrorType`` is set to ``replicas``.
